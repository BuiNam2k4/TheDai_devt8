...
public class AIGradingService {
        ...
        public void gradeSubmission(Long submissionId) throws Exception {
                ...
                // 2. Nếu bài học chưa có rubric, thử trích xuất từ file đáp án
                if (rubrics == null || rubrics.isEmpty()) {
                        log.info("Rubrics not found for lesson {}. Extracting rubrics from solution...",
                        ...
                        if (extracted != null && !extracted.isEmpty()) {
                                ...
                                throw new AppException(ErrorCode.RUBRIC_NOT_FOUND,
                                                "Bài học này chưa có Barem chấm điểm hợp lệ. Vui lòng liên hệ Giáo viên kiểm tra lại bài học.");
                        }
                }
        ...
        private void gradeWithRubrics(Submission submission, List<Rubric> rubrics) throws Exception {
                ...
                if (submission.getAnswerFileUrl() != null && !submission.getAnswerFileUrl().isBlank()) {
                        try {
                                studentFileBytes = restTemplate.getForObject(submission.getAnswerFileUrl(),
                                                byte[].class);
                                ...
                                log.info("Downloaded student answer file ({} bytes, mime: {}) for submission {}",
                                                studentFileBytes != null ? studentFileBytes.length : 0, mimeType,
                                                submission.getId());
                        } catch (Exception e) {
                                log.warn("Could not download student answer file from URL [{}]: {}",
                ...
                if (studentFileBytes != null && studentFileBytes.length > 0) {
                        rawAiResponse = geminiService.callGeminiWithMedia(promptBuilder.toString(), studentFileBytes,
                                        mimeType);
                } else {
                ...
                for (int i = 0; i < results.size(); i++) {
                        ...
                        Rubric matchedRubric = rubrics.stream()
                                        .filter(r -> result.getRubricId() != null
                                                        && r.getId().equals(result.getRubricId()))
                                        .findFirst()
                                        .orElseGet(() -> rubrics.stream()
                                                        .filter(r -> result.getRubricId() != null
                                                                        && Long.valueOf(r.getStepOrder())
                                                                                        .equals(result.getRubricId()))
                                                        .findFirst()
                                                        .orElseGet(() -> index < rubrics.size() ? rubrics.get(index)
                                                                        : rubrics.get(0)));
