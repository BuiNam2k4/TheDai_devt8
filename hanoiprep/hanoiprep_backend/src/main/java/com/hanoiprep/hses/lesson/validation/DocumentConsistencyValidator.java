 ...
 * 1. Đề bài (questionFile) vs Đáp án (solutionFile): Đáp án phải giải cho đề
 * bài tương ứng
 * 2. Tài liệu lý thuyết (materialFile) vs Đề bài: Cùng môn học / lĩnh vực kiến
 * thức
...
public class DocumentConsistencyValidator {
    ...
    private static final int MAX_ANALYSIS_CHARS = 25000;
    private static final int MAX_MATERIAL_CHARS = 10000;
    ...
    public DocumentValidationResult validate(
        ...
        if (questionText.isBlank()) {
            if (questionFile == null || questionFile.isEmpty()) {
                return DocumentValidationResult.failure(
                        "Tệp Đề bài bị rỗng hoặc chưa được chọn. Vui lòng tải lên file đề bài.",
                        List.of("questionFile"));
            }
            log.warn("Không bóc tách được nội dung chữ từ file Đề bài [{}], sử dụng tên file làm dữ liệu dự phòng.", questionFile.getOriginalFilename());
            questionText = "Đề bài bài học: " + questionFile.getOriginalFilename();
        }
        ...
        if (solutionText.isBlank()) {
            if (solutionFile == null || solutionFile.isEmpty()) {
                return DocumentValidationResult.failure(
                        "Tệp Đáp án bị rỗng hoặc chưa được chọn. Vui lòng tải lên file đáp án.",
                        List.of("solutionFile"));
            }
            log.warn("Không bóc tách được nội dung chữ từ file Đáp án [{}], sử dụng tên file làm dữ liệu dự phòng.", solutionFile.getOriginalFilename());
            solutionText = "Hướng dẫn giải & Đáp án: " + solutionFile.getOriginalFilename();
        }
        ...
        // Rút gọn văn bản tối ưu cho AI phân tích (nếu vượt quá ngưỡng tối đa)
        ...
        String truncatedMaterial = materialText.isBlank() ? "" : textNormalizer.truncateForAnalysis(materialText, MAX_MATERIAL_CHARS);
        ...
        try {
            ...
            log.warn("Dịch vụ thẩm định AI tạm thời không khả dụng: {}. Cho phép tiếp tục lưu bài học...", e.getMessage());
            // Fallback an toàn để không chặn giáo viên lưu bài học khi dịch vụ AI gặp sự cố tạm thời
            return DocumentValidationResult.builder()
                    .valid(true)
                    .reason("Đã tải lên tệp tin bài học (bỏ qua thẩm định ngữ nghĩa do AI phản hồi chậm).")
                    .mismatchedFiles(List.of())
    ...
    private String buildValidationPrompt(String questionText, String solutionText, String materialText) {
        boolean hasMaterial = materialText != null && !materialText.isBlank();
        StringBuilder sb = new StringBuilder();

        sb.append(
                """
                        Bạn là Chuyên gia Thẩm định Học liệu Giáo dục cao cấp của hệ thống HanoiPrep.
                        Nhiệm vụ của bạn là kiểm tra tính ĐỒNG NHẤT, ĐÚNG VAI TRÒ và TƯƠNG THÍCH HOÀN TOÀN giữa các tệp tin học liệu tải lên:
                        1. Tệp Đề bài (`questionFile`) - BẮT BUỘC
                        2. Tệp Đáp án (`solutionFile`) - BẮT BUỘC
                        """);

        if (hasMaterial) {
            sb.append("3. Tệp Tài liệu học tập / Lý thuyết (`materialFile`) - ĐÍNH KÈM\n\n");
        } else {
            sb.append(
                    "\nLƯU Ý ĐẶC BIỆT: Người dùng KHÔNG tải lên Tệp Tài liệu lý thuyết (`materialFile`). Tệp này là TÙY CHỌN (Optional), hoàn toàn hợp lệ. TUYỆT ĐỐI KHÔNG bắt lỗi thiếu tệp `materialFile` hay coi việc không có `materialFile` là vi phạm.\n\n");
        }
        ...
        if (hasMaterial) {
            sb.append(
                    """
                            - [Tệp 3 - materialFile (TÀI LIỆU HỌC TẬP / LÝ THUYẾT)]:
        ...
        sb.append(
                """
                        - Nếu các tệp tin bị nộp trùng lặp nội dung giống hệt nhau -> `valid: false`, reason: "Phát hiện tệp tin bị tải trùng lặp nội dung giữa các ô."

                        QUY TẮC 3: ĐỒNG NHẤT MÔN HỌC & CHUYÊN ĐỀ (SUBJECT & TOPIC CONSISTENCY)
                        - Các tệp tin tải lên BẮT BUỘC phải cùng một môn học (Toán, Lý, Hóa, Văn, Tiếng Anh...), cùng cấp độ và cùng một chủ đề kiến thức.
                        - KHÔNG chấp nhận: Đề bài là môn Toán nhưng Đáp án là môn Hóa học; hoặc Đề bài gồm 3 bài toán hình học nhưng Đáp án lại giải các câu đại số hoàn toàn khác.
        ...
        sb.append(
                """
                        ...
                        Hãy trả về DUY NHẤT một đối tượng JSON có cấu trúc sau:
                        {
                          ...
                          "mismatchedFiles": ["Tên các file không khớp, ví dụ: 'solutionFile', 'questionFile' hoặc để mảng rỗng [] nếu hợp lệ"]
                        }
                        """);
    ...
    private DocumentValidationResult parseAiResponse(
        ...
        try {
            ...
            String reason = parsed.path("reason")
                    .asText(valid ? "Tài liệu đồng nhất và hợp lệ" : "Tài liệu không khớp nhau");
            ...
            if (mismatchArray.isArray()) {
                for (JsonNode item : mismatchArray) {
                    String fileKey = item.asText();
                    // Nếu người dùng không nộp materialFile thì bỏ qua cảnh báo liên quan đến
                    // materialFile
                    if (materialText.isBlank() && "materialFile".equalsIgnoreCase(fileKey)) {
                        continue;
                    }
                    mismatchedFiles.add(fileKey);
                }
            }

            // Nếu chỉ báo lỗi do thiếu materialFile mà người dùng không upload materialFile
            // -> coi là hợp lệ
            if (materialText.isBlank() && !valid && mismatchedFiles.isEmpty()) {
                valid = true;
                reason = "Đề bài và Đáp án đồng nhất và hợp lệ.";
            }
